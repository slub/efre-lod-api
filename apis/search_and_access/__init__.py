...
api = Namespace(name="search and access", path="/", description="Elasticsearch Search and Access Operations")
...
class searchDoc(Resource):
    ...
    def get(self,entity_type):
        ...
        if entity_type in get_indices():
                ...
                res=self.es.search(index=entity_type,body=search,size=args.get("size"), from_=args.get("from"))
    ...
    def get(self,entity_type,id):
        ...
        try:
            ...
            if entity_type=="resources": 
                ...
                typ="schemaorg"
            res=self.es.get(index=entity_type,doc_type=typ,id=name,_source_exclude=self.excludes)
...
class ESWrapper(Resource):
    ...
    def get(self):
        ...
        res=self.es.search(index=searchindex,body=search,size=args["size"],from_=args["from"],_source_exclude=self.excludes)
        if "hits" in res and "hits" in res["hits"]:
            for hit in res["hits"]["hits"]:
                retarray.append(hit.get("_source"))
        return output.parse(retarray, args.get("format"), "", request)
