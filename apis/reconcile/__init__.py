...
api = Namespace("reconcile",path="/reconcile/",description="Openrefine Reconcilation and Data Extension Operations")
...
class ProposeProperties(Resource):
    ...
    es_host, es_port, excludes, indices =load_config("apiconfig.json", "es_host","es_port","excludes","indices")
    ...
    def get(self):
        ...
        if args["limit"]:
            ...
            if typ in self.indices:
                ...
                mapping = self.es.indices.get_mapping(index=entity) # some python magic to get the first element of the dictionary in indices[typ]
...
class SuggestEntityEntryPoint(Resource):
    ...
    def get(self):
        ...
        """
        args = self.parser.parse_args()
        search = self.es.search(index=",".join(get_indices()[:-1]),body={"query":{"match_phrase_prefix":{"name":{"query":args["prefix"]}}}},_source_include=["name","@type"])
        result = {"result":[]}
...
class SuggestPropertyEntryPoint(Resource):
    ...
    def get(self):
        ...
        args = self.parser.parse_args()
        result = {"result":[]}
        mapping = self.es.indices.get_mapping(index=get_indices()[:-1]) # some python magic to get the first element of the dictionary in indices[typ]
...
class FlyoutEntityEntryPoint(Resource):
    ...
    def get(self):
            ...
            doc = self.es.get(index=arg["id"].split("/")[0],id=arg["id"].split("/")[1],doc_type="schemaorg",_source_include="name")
            ret = {"html": "<p style=\"font-size: 0.8em; color: black;\">{}</p>".format(doc["_source"]["name"]),"id":arg["id"]}
            return jsonpify(ret)
...
class FlyoutTypeEntryPoint(Resource):
    ...
    def get(self):
            ...
            """
            arg = self.parser.parse_args()
            ret = {"html": "<p style=\"font-size: 0.8em; color: black;\">{}</p>".format(self.indices[arg["id"]]["description"]),"id":arg["id"]}
            return jsonpify(ret)
...
class apiData(Resource):
    ...
    es_host, es_port, excludes, indices, base =load_config("apiconfig.json", "es_host","es_port","excludes","indices","base")
    es = Elasticsearch([{'host':es_host}],port=es_port,timeout=10)
    @api.response(200,'Success')
                ...
                for _id in data.get("ids"):
                    ...
                    for index in self.indices:
                        if _id.split("/")[0]==self.indices[index]["index"]:
                            ...
                            break
                    es_data = self.es.get(index=_id.split("/")[0],doc_type="schemaorg",id=_id.split("/")[1],_source_include=source)
